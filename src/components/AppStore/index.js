/*
react import should occur before import of ../TabItem/index," 
is that there is an issue with the order of your import statements. 
In JavaScript/React, it is generally 
recommended to import external 
dependencies like React (or any third-party libraries) 
...
const appsList = [
  {
    appId: 0,
    appName: 'Facebook', // alt attribute for image
    imageUrl:
...
class AppStore extends Component {
  state = {ActiveTabId: 'SOCIAL', searchInputEl: ''}
  ...
  searchEvent = event => {
    ...
    this.setState({searchInputEl: searchValue})
  }

  render() {
    const {ActiveTabId, searchInputEl} = this.state
    console.log(searchInputEl)
    const initialItems = appsList.filter(
      eachFilterObjects => eachFilterObjects.category === ActiveTabId,
    )

    const finalListApps = initialItems.filter(eachSearch =>
      eachSearch.appName.toLowerCase().includes(searchInputEl),
    )
    ...
    return (
      <div className="bg">
        ...
        <ul className="ulTabItems">
          {finalListApps.map(eachObjectItem => (
            <AppItem
              eachObjectItemProp={eachObjectItem}
              key={eachObjectItem.appId}
            />
          ))}
        </ul>
      </div>
